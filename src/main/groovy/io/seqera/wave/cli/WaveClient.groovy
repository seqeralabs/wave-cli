...
/**
 * Simple client for Wave service
 *
...
class WaveClient {
    ...
    private static final Logger log = LoggerFactory.getLogger(WaveClient)

    final static private String[] REQUEST_HEADERS = [
    ...
    final static private List<Integer> SERVER_ERRORS = [249, 502, 503, 504]
    ...
    ContainerInspectResponse inspect(ContainerInspectRequest request) {
        ...
        try {
            ...
            if (resp.statusCode() == 200)
                ...
                String msg = "Wave invalid response: [${resp.statusCode()}] ${resp.body()}"
    ...
    WaveClient withEndpoint(String endpoint) {
    ...
    ContainerStatusResponse awaitCompletion(String requestId, Duration await) {
        ...
        while (true) {
            ...
            if (System.currentTimeMillis() - startTime > await.toMillis()) {
                String msg = "Container provisioning did not complete within the max await time (${await.toString()})"
